// main.js - prototype
// Classes: Game, Player, MapGrid, EventManager, SimpleAI, MiniGames, UI, AudioSynth
// Saves: This is a single-file prototype intended to be dropped alongside index.html
...
class MiniGames {
  ...
  update(){
    ...
    if(this.active==='rally'){ 
      ...
      s.elapsed++;
      if(s.elapsed>600){
        this.resolve({success:false,reason:'timeout'}); this.active=null;
      }
    } else if(this.active==='espionage'){ 
      let s=this.state;
      if(s.phase==='show'){ if(--s.showTimer<=0) s.phase='play' }
    }
  }
  ...
  onClickTile(idx){
    ...
    if(idx===s.seq[s.stage]){
      s.stage++;
      if(s.stage>=s.seq.length){
  ...
  render(ctx, x, y, w, h){
    ...
    if(this.active==='rally'){
      let s=this.state;
      ctx.fillStyle="#222"; ctx.fillRect(20,20,w-40,24);
      let gs = 20 + (w-40)*s.greenStart, ge = 20 + (w-40)*s.greenEnd;
      ctx.fillStyle="#2ecc71"; ctx.fillRect(gs,20,ge-gs,24);
      ...
      let s=this.state;
      ctx.fillStyle="#fff"; ctx.fillText("Espionage sequence:",10,18);
      if(s.phase==='show'){ 
        ctx.fillStyle="#ffcc00"; ctx.fillText(s.seq.map(_=>'▢').join(' '),10,40);
        ctx.fillStyle="#999"; ctx.fillText("Memorize the pattern...",10,60);
      } else {
        for(let i=0;i<4;i++){
          ctx.fillStyle="#333"; ctx.fillRect(10 + i*40,30,32,32);
          ctx.fillStyle="#fff"; ctx.fillText(i+1,22 + i*40,52);
        }
        ctx.fillStyle="#999"; ctx.fillText("Click tiles in order (1-4) on the map",10,80);
      }
    }
    ctx.restore();
  }
  ...
  render(){
    ...
    for(let y=0;y<mg.rows;y++){
      for(let x=0;x<mg.cols;x++){
        ...
        if(mg.isControlled(x,y)){
          ctx.strokeStyle = "#fffb99"; ctx.lineWidth=2;
          ctx.strokeRect(x*ts+2, y*ts+2, ts-4, ts-4);
        }
        if(t.type==='city'){ 
          ctx.fillStyle="#2b2b2b";
          ctx.fillRect(x*ts+ts/4, y*ts+ts/4, ts/2, ts/2);
        } else if(t.type==='village'){ 
          ctx.fillStyle="#523515";
          ctx.fillRect(x*ts+ts/3, y*ts+ts/3, ts/3, ts/3);
        }
      }
    }
    for(let n of g.ai.npcs){
      ctx.fillStyle = n.loyal ? "#2ecc71" : "#e74c3c";
      ctx.fillRect(n.x*ts + ts/3, n.y*ts + ts/3, ts/3, ts/3);
    }
    ctx.save();
    ctx.fillStyle="#000"; ctx.globalAlpha=0.35;
    ctx.fillRect(0, this.canvas.height-56, this.canvas.width,56);
    ctx.globalAlpha=1;
    ctx.fillStyle="#fff";
    let p = g.player;
    ctx.fillText(`Day ${p.day}`, 8, this.canvas.height-36);
    ctx.fillText(`Propaganda: ${p.propaganda}`, 8, this.canvas.height-20);
    ctx.fillText(`Military: ${p.military}`, 170, this.canvas.height-36);
    ctx.fillText(`Economy: ${p.economy}`, 170, this.canvas.height-20);
    ctx.fillText(`Morale: ${p.morale}`, 340, this.canvas.height-36);
    ctx.fillText(`Territory: ${g.map.controlled.size}`, 340, this.canvas.height-20);
    ctx.restore();
    if(g.minigames.active){
      g.minigames.render(this.ctx, 8,8,380,100);
    }
  }
}
...
class Game {
  constructor(canvas){
    ...
    this.lastEvent = null;
    this.sfx = new AudioSynth();
  }

  _bind(){
    window.addEventListener('keydown', (e)=>{ this.minigames.onKey(e) });
    this.canvas.addEventListener('click', (ev)=>{
      if(this.minigames.active==='espionage'){
        let rect = this.canvas.getBoundingClientRect();
        let x = Math.floor((ev.clientX-rect.left)/this.map.tileSize);
        let zone = (x % 4);
        this.minigames.onClickTile(zone);
      }
    });
  }
  ...
  advanceDay(){
    let p = this.player;
    p.day++;
    let ev = this.events.drawEvent();
    if(ev){
      this.eventCurrent = ev;
      this.lastEvent = ev;
    } else {
      p.propaganda = Math.max(0,p.propaganda + Utils.randInt(-3,3));
      p.economy = Math.max(0,p.economy + Utils.randInt(-4,4));
      p.military = Math.max(0,p.military + Utils.randInt(-2,2));
      p.morale = Math.max(0, Math.min(100, p.morale + Utils.randInt(-2,2)));
    }
    p.score = Math.round((p.propaganda + p.military + p.economy + p.morale) * this.map.controlled.size / Math.max(1,p.day));
    this.sfx.beep(220,0.05);
    this.checkFailure();
  }

  chooseEvent(choiceIndex){
    if(!this.eventCurrent) return;
    let choice = this.eventCurrent.choices[choiceIndex];
    this.events.applyChoice(choice);
    if(this.eventCurrent.id==='speech' && Math.random()<0.6){
      this.minigames.startRally().then(res=>{
        if(res.success){ this.player.morale = Math.min(100,this.player.morale + 10); this.player.propaganda = Math.max(0,this.player.propaganda - 2); this.sfx.beep(880,0.12) }
        else { this.player.morale = Math.max(0,this.player.morale - 8); this.sfx.beep(120,0.12) }
      })
    } else if(this.eventCurrent.id==='spy' && Math.random()<0.6){
      this.minigames.startEspionage().then(res=>{
        if(res.success){ this.player.military += 6; this.sfx.beep(660,0.12) } else { this.player.propaganda = Math.max(0,this.player.propaganda - 4); this.sfx.beep(160,0.12) }
      })
    }
    this.eventCurrent = null;
  }
  ...
  checkFailure(){
    let p = this.player;
    if(p.morale <= 0){
      this.endGame("Empire of Enthusiasm collapses — morale hit rock bottom. Your absurd policies are rejected in a spectacular pie fight.");
    } else if(p.economy <= 0){
      this.endGame("Economic meltdown — the grand parade's floats were actually paper mache. The treasury is empty and you must retire to interpretive dance.");
    } else if(this.map.controlled.size <= 0){
      this.endGame("You lost every tile to baffled resistance. Your minions defect to a traveling circus.");
    }
  }
...
window.addEventListener('load',()=>{
  ...
  function renderSidebar(){
    ...
    if(game.eventCurrent){
      ...
      ev.choices.forEach((c,i)=>{
        html += `<button class="choice" data-idx="${i}">${c.t}</button>`;
      });
      eventDiv.innerHTML = html;
      // bind choice buttons
      eventDiv.querySelectorAll('button.choice').forEach(btn=>{
        btn.onclick = ()=>{ game.chooseEvent(parseInt(btn.dataset.idx)); renderSidebar(); };
      });
    } else {
      eventDiv.innerHTML = `<div><strong>No immediate events.</strong></div>`;
    }

    // mini-game info
    if(game.minigames.active){
      minigameDiv.innerHTML = `<div><strong>Mini-game active:</strong> ${game.minigames.active}</div>`;
    } else {
      minigameDiv.innerHTML = `<div><strong>No mini-game active.</strong></div>`;
    }
  }
  ...
  // keyboard hint for rally
  window.addEventListener('keydown', (e)=>{
    if(e.code==='Space' && game.minigames.active==='rally'){ 
      game.minigames.onKey(e);
      renderSidebar();
    }
  });

  // update loop for DOM (not heavy)
